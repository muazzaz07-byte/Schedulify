...
def manage_blocked():
    if 'user_id' not in session or session.get('role') != 'lecturer':
        return redirect(url_for('login'))
    slots = [{"start_date": "1 April 2026", "end_date": "30 April 2026", "start_time": "11:00 AM", "end_time": "1:00 PM", "days": "MON, THU, FRI"}]
    return render_template('lecturer/unblock_lecturer.html', blocked_slots=slots)
...
def cancellation():
    ...
    # FIXED: Querying both confirmed AND cancelled logs, pushing cancellations to the back
    all_eligible_bookings = Booking.query.join(ScheduleSlot).filter(
        Booking.student_id == user_id,
        Booking.status.in_(['CONFIRMED', 'CANCELLED_BY_STUDENT', 'CANCELLED_BY_LECTURER'])
...
def appointments():
    ...
    if user_role == 'lecturer':
        ...
        # Column 2: Bookings cancelled by student or lecturer
...
def get_weekly_chances():
    ...
    user_id = session['user_id']
    weekly_bookings_count = Booking.query.filter(
        Booking.student_id == user_id,
...
def book_appointment():
    ...
    try:
        ...
        # FIXED: Create and inject the system alert log tracking block for the lecturer's view panel
...
#-------------------------------------------------------------------------------API BLOCK SLOTS (FIXED TIME INTERVALS)-------
...
def get_lecturer_slots():
    ...
    try:
        # Fetch all schedule slots generated by the logged-in lecturer
        slots = ScheduleSlot.query.filter_by(lecturer_id=lecturer_id).all()
        
        slots_data = {}
        for slot in slots:
            dt = slot.date_time
            # FIXED: Manually building the ISO key string to enforce trailing padding
            # This completely guarantees it will perfectly match the JavaScript format: "YYYY-MM-DDTHH:MM:00"
            ...
            if slot.booking_slots:
                # Find the active confirmed booking linked to this slot row if it exists
...
#-------------------------------------------------------------------------------STUDENT API CANCEL AAPOINTMENT----------------------
...
#-------------------------------------------------------------------------------LECTURER API CANCEL AAPOINTMENT----------------------
