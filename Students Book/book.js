...
function submitBooking() {
    // Collect the data from your variables (stored when student clicked the slot)
    const bookingData = {
        lecturer_name: document.getElementById('confirm-name').innerText,
        date: selectedDate, // You must define these when a slot is clicked
        time: selectedTime
    };
    ...
    .then(data => {
        if(data.success) {
            showSuccess(); // Show the success modal only if DB update worked
        } else {
