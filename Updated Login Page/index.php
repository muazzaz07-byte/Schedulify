...
// --- 1. DIRECT LOGOUT HANDLING ---
// If the user arrives here via index.php?logout=true, wipe the session
if (isset($_GET['logout']) && $_GET['logout'] == 'true') {
    session_unset();    // Remove all session variables
    session_destroy();  // Destroy the session
    header("Location: index.php"); // Refresh to clear the URL parameter
    exit();
}

// --- 2. ERROR & FORM STATE RETRIEVAL ---
$errors = [
    'login' => $_SESSION['login_error'] ?? '',
    'register' => $_SESSION['register_error'] ?? ''
];

// Determine which form (login or register) should be visible on load
$activeForm = $_SESSION['active_form'] ?? 'login';

// Retrieve registration data to refill the form if an error occurred
$reg_data = [
...
// --- 3. SESSION CLEANUP ---
// Clear errors so they don't reappear if the user refreshes the page
unset($_SESSION['login_error'], $_SESSION['register_error'], $_SESSION['active_form']);

/**
 * Generates an HTML message with the appropriate CSS class
 */
function showMessage($message) {
    if (empty($message)) return '';
    
    // Check if the message contains "successful" to apply green styling
    $isSuccess = (strpos(strtolower($message), 'successful') !== false);
    $class = $isSuccess ? 'msg-success' : 'msg-error';
    
    return "<p class='$class'>" . htmlspecialchars($message) . "</p>";
}
?>
