...
function saveSecurityQuestion() {
    ...
    document.getElementById('security-answer').value = "";
        ...
        function showPage(pageId) {
    // Senarai semua ID div halaman anda
    const pages = [
        'page-profile', 
        'page-customize', 
        'page-password', 
        'page-security-q', 
        'page-edit-username', 
        'page-help' // Pastikan ID ini betul
    ];

    pages.forEach(id => {
        const p = document.getElementById(id);
        if (p) {
            p.classList.add('hidden'); // Sembunyikan semua
        }
    });

    const target = document.getElementById(pageId);
    if (target) {
        target.classList.remove('hidden'); // Tunjukkan yang dipilih sahaja
